		...
		(function() {
			...
			async function updateChat() {
				...
				try{
					...
					//this is not even sligthly accurate, but it's good enough for the moment
					let l1 = $.parseHTML(incoming.html()).length;
					let l2 = $.parseHTML(response).length;
					if(!(l1===l2)){
						console.log(l1);
						console.log(l2);
					}
					if (l1 !== l2 && $("#incoming:hover").length != 1) {
			...
			async function updateLog(){
				if(window.num !== undefined){
					...
					try{
						...
						let log = $("#log");
						let l1 = $.parseHTML(log.html()).length;
						let l2 = $.parseHTML(response).length;
			...
			async function updateNews(){
				...
				try{
					...
					var news = $("#news");
					let l1 = $.parseHTML(news.html()).length;
					let l2 = $.parseHTML(response).length;
			...
			var buildCommand = function(commandType, params) {
				...
				else if (commandType == "abandonOption1"){
					 for (var i = 0; i < filteredSquares.length; i++) {
						var square = filteredSquares[i];
